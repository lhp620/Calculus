// app.js — AP Calculus AB/BC Interactive Learning SPA
...
// BC badge HTML
const BC_BADGE = '<span style="font-size:0.65rem;font-weight:700;padding:2px 6px;border-radius:8px;background:rgba(236,72,153,0.15);color:#ec4899;border:1px solid rgba(236,72,153,0.3);margin-left:6px;vertical-align:middle">BC</span>';
    ...
    <div class="unit-hero">
      <div class="container">
        ...
        <div class="unit-hero-header">
          ...
          <div>
            <h1>${unit.title}${unit.bc ? '&nbsp;' + BC_BADGE : ''}</h1>
          </div>
        </div>
          ...
          return `
            <a href="#lesson/${lesson.id}" class="lesson-item ${isMastered ? 'completed' : ''}" style="--unit-accent:${accent}">
              ...
              <div class="lesson-info">
                <h4>${lesson.title}${lesson.bc ? '&nbsp;' + BC_BADGE : ''}</h4>
                <p>${lesson.desc}</p>
              </div>
...
async function renderLessonPage(lessonId) {
  ...
  const lesson = await getLessonData(unitId, lessonId);
      ...
      <div class="lesson-container">
        ...
        <div class="lesson-header">
          <div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            ${lessonMeta.bc ? '<span style="font-size:0.8rem;font-weight:700;padding:3px 10px;border-radius:12px;background:rgba(236,72,153,0.12);color:#ec4899;border:1px solid rgba(236,72,153,0.3)">BC Only</span>' : ''}
          </div>
  ...
  main.innerHTML = `
    <div style="padding:48px 0 80px">
      <div class="container">
        ...
        <p style="color:var(--color-text-muted);margin-bottom:32px">All key AP Calculus AB/BC formulas. Search by topic, unit, or keyword.</p>
        <div class="formula-search">
          <span class="search-icon">🔍</span>
          <input type="text" id="formula-search-input"
                 placeholder="Search formulas… (e.g. chain rule, integral, series)"
        ...
        <div id="formula-grid" class="formula-grid">
          ${formulas.map(f => `
            <div class="formula-card" data-keywords="${(f.name + ' ' + (f.tags || []).join(' ') + ' ' + f.unit).toLowerCase()}">
              <h4>${f.unit} — ${f.name}${(f.tags || []).includes('BC') ? '&nbsp;' + BC_BADGE : ''}</h4>
